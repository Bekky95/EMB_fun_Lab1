 ...
 *   
 *   Functions: TIMER2_IRQHandler
 *   manuelle Berechnung Baudrate
 *********************************************************************/
//Initialisierung der Variablen für TIMER2 IRQHandler
unsigned int TimeInterval2, new_TimeValue2, old_TimeValue2;
int new_TimeInterval2;
int Not_First_Measurement2 = 0;
unsigned int MinimumMeasurement2 = 0x0FFFFFFF; // dec: 268.435.455
float baudrate=0;

void	TIMER2_IRQHandler(void){
	if ( LPC_TIM2->IR & 0x20){      					// In Channel 1 Interrupt?
		
		old_TimeValue2 = new_TimeValue2;				// Alter Timer Wert überschreiben
		new_TimeValue2 = LPC_TIM2->CR1;					// Capture Reg Inhalt (TC-val bei event) gespeichert

		if ( Not_First_Measurement2 ) { 				// erste Messung?
																						// Timer intervall berechnen:
			new_TimeInterval2 =  new_TimeValue2 - old_TimeValue2;

			if ( new_TimeInterval2 < MinimumMeasurement2){
																						// minimales Zeitintervall kleiner als Vorheriges?
				MinimumMeasurement2 = new_TimeInterval2;
			}
		}
		else {
			Not_First_Measurement2 = 1;
		}
    LPC_TIM2->IR = 0x20;  									// Clear interrupt, Interrupt bestätigt und den Interrupt-Status löschen
	}	

 baudrate =(25000000)/(float)MinimumMeasurement2;	// Baudrate = Frequenz/(typecast to float)minimales Zeitintervall
}
...
void init_Timer0_for_UART(void) {
	...
	LPC_SC->PCONP |= (1 << 1);								// TIMER0 Peripheriemodul einschalten
	
	LPC_TIM0->PR = (2500000-1);								// Prescaler max_val: 250000-1 für 10sek Intervall        
	LPC_TIM0->MR0 = 10;												// (25MHz*10^6 * 10) / (10+1)
	
	LPC_TIM0->MCR |= 3;												// Bit0 = 1 = Interrupt an ; Bit1 = 1 = Reset bei Interrupt
	LPC_TIM0->TCR |= 3;												// Bit0 = 1 = enable counter, Bit1 steigende Flanke zählen
	LPC_TIM0->TCR &= ~(1<<1);									// Bit1 = 0,  Counter reset 
	
}

void init_Timer2_for_UART(void){
	// Put your code here:
	
	LPC_SC->PCONP |= (1 << 22); 							// TIMER2 Peripheriemodul einschalten
	LPC_PINCON->PINSEL0 |= (3<<10);						// konfigurieren des Pins/CAP 2.1 - Port 0 Pin 5 (0.5)
																						// Bit 10 & 11 auf 1 -> Eingänge -> beide für Capture benötigt
	LPC_PINCON->PINSEL0 |= (1<<4);							// TXD0 Function init -> P0.2 
	LPC_PINCON->PINSEL0 |= (1<<6); 							// RXD0 Function init -> P0.3 (auto baud)
	
	
	LPC_TIM2->IR = 0x20;											// Interrup Register: cr1 interrupt setzen
																						// Bit 1 = 1 -> MR1 Interrupt -> match Channel 1 (IR-source)
	LPC_TIM2->CTCR = 0x04;										// Count Control Register.: 0000 0100 (Timer || Counter Mode?)
																						// Bit 3:2 = 01 -> CAP2.1 für Timer 2 - Counter Mode
	LPC_TIM2->CCR = 0x38;											// Capture Control Register - 0011 1000 (Welche edge, interrupt generieren?
																						// Bits 5:3 = 1
																						// Bit 3 = 1: capture on rising edge - CR1 loaded with TC
																						// Bit 4 = 1: capture on falling edge - CR1 loaded with TC
																						// Bit 5 = 1: Interrupt on CAP2.1 event
	LPC_TIM2->TCR = 0x02;  										// Timer Control Register - 0010 
																						// Bit 1 = 1: TimerCounter und PrescaleCounter zurücksetzen auf pos edge
	LPC_TIM2->TCR = 0x01;											// Bit 0 = 1: Timer-/Prescale Counter starten
																						// LPC_TIM2->TCR = 0x03; GEHT NICHT!!!!
}

void init_NVIC(void){
	// Put your code here:
		
	NVIC_EnableIRQ(TIMER0_IRQn);							// Interrupts für Timer0 aktivieren
	NVIC_SetPriority(TIMER0_IRQn, 1); 				// Timer Priority auf 1
	
	NVIC_EnableIRQ(UART0_IRQn);								// Interrupt für UART0 aktivieren
	NVIC_SetPriority(UART0_IRQn, 2);					// Prio 2

	NVIC_EnableIRQ(TIMER2_IRQn);							// Interrupts für Timer2 aktivieren
	NVIC_SetPriority(TIMER2_IRQn, 2);					// Prio 2
}
